...
// The FASHN provider (Phase 0.5). One garment layer per call: dress `person`
// in `garment`, store the result at `out`. It runs inside a request, so the
// storage helpers (user-scoped, RLS) resolve and write the user's own paths.
//
// FASHN_API_KEY is read at call time, backend only. FASHN_MODEL defaults to the
// quality try-on model; mode is always "quality" — there is no standard tier.
...
const COST_ESTIMATE_USD = 0.3; // ~max-tier layer cost; informational only
      ...
      const res = await fetch(RUN_URL, {
        ...
        body: JSON.stringify({
          ...
          inputs: {
            ...
            category: fashnCategory(input.category),
